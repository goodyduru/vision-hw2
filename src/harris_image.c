...
image structure_matrix(image im, float sigma)
{
    ...
    for ( i = 0; i < im.w; i++ ) {
        for ( j = 0; j < im.h; j++ ) {
            pixel = pow(get_pixel(gx, i, j, 0), 2);
            set_pixel(S, i, j, 0, pixel);
                
            pixel = pow(get_pixel(gy, i, j, 0), 2);
            set_pixel(S, i, j, 1, pixel);
                
            pixel = get_pixel(gx, i, j, 0) * get_pixel(gy, i, j, 0);
            set_pixel(S, i, j, 2, pixel);
        }
    } 
...
image cornerness_response(image S)
{
    ...
    for ( i = 0; i < S.w; i++ ) {
        for ( j = 0; j < S.h; j++ ) {
            trace = get_pixel(S, i, j, 0) + get_pixel(S, i, j, 1);
            det = (get_pixel(S, i, j, 0) * get_pixel(S, i, j, 1)) - pow(get_pixel(S, i, j, 2), 2);
            set_pixel(R, i, j, 0, det - alpha * pow(trace, 2));
        }
    } 
    return R;
}
